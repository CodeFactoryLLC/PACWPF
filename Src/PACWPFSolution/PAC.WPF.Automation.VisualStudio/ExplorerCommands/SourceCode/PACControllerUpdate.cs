...
namespace PAC.WPF.Automation.VisualStudio.ExplorerCommands.SourceCode
{
    ...
    /// </summary>
    public class PACControllerUpdate : CSharpSourceCommandBase
    {
        private static readonly string commandTitle = "PAC Controller Update";
        private static readonly string commandDescription = "Confirms all members are implemented and rebuilds subscriptions to all contracts that are registered as fields on the controller.";

#pragma warning disable CS1998

        /// <inheritdoc />
        public PACControllerUpdate(ILogger logger, IVsActions vsActions) : base(logger, vsActions, commandTitle, commandDescription)
        {
            //Intentionally blank
        }
        ...
        public override async Task<bool> EnableCommandAsync(VsCSharpSource result)
        {
            ...
            try
            {
                ...
                isEnabled = result.SourceCode.Classes.Any(c => c.ClassInterfaceWithBaseInterface("PAC.IController"));
            }
